...
export const isDict = (v: Value): boolean =>
    isEmptySExp(v) || (isValidDictList(v) && !hasDuplicateKeys(v));

export const isValidDictList = (v: Value): boolean => {
    return isEmptySExp(v) ? true :
        isCompoundSExp(v) ? isValidDictPairs(v) :
        false;
};

const isValidDictPairs = (v: CompoundSExp): boolean => {
    const pair = v.val1;
    ...
    const rest = v.val2;
    return isEmptySExp(rest) ? true :
           isCompoundSExp(rest) ? isValidDictPairs(rest) :
           false;
};

export const hasDuplicateKeys = (dict: Value): boolean => {
    const seen: string[] = [];
