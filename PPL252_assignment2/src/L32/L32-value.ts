...
export type SExpValue = number | boolean | string | PrimOp | Closure | SymbolSExp | EmptySExp | CompoundSExp | DictValue; // YuVal
...
export type DictValue = {tag: "DictValue"; table: { [key: string]: Value };}; // YuVal
...
export const valueToString = (val: Value): string =>
    ...
    isDictValue(val) ? `(dict ${Object.entries(val.table).map(([k, v]) => `(${k} ${valueToString(v)})`).join(" ")})` : //YUVAL

    val;

export const makeDictValue = (pairs: [string, Value][]): DictValue => { // YuVal
    const table: { [key: string]: Value } = {};
    for (const [k, v] of pairs) {
        table[k] = v;
    }
    return { tag: "DictValue", table };
};

export const isDictValue = (x: any): x is DictValue => // YuVal
    typeof x === "object" &&
    x !== null &&
    x.tag === "DictValue" &&
    typeof x.table === "object";
