...
import { DictExp, isDictExp } from "./L32-ast"; // YuVal
import { DictValue, isSymbolSExp, makeDictValue, isDictValue } from "./L32-value"; //YUVAL
// ========================================================
// Eval functions
...
const L32applicativeEval = (exp: CExp, env: Env): Result<Value> =>
    ...
    isDictExp(exp) ? evalDictExp(exp, env) : // YuVal
    exp;
...
const L32applyProcedure = (proc: Value, args: Value[], env: Env): Result<Value> =>
    ...
    isDictValue(proc) ? // YuVal
        (args.length === 1 && isSymbolSExp(args[0]) ? (proc.table.hasOwnProperty(args[0].val) ?
        makeOk(proc.table[args[0].val]) :
        makeFailure(`Key '${args[0].val}' not found in dict`)) :
        makeFailure("Dict expects a single symbol as key")) :
...
export const evalDictExp = (exp: DictExp, env: Env): Result<DictValue> => {
    const evaluatedPairsResult = mapResult(
        ([k, vExp]) => bind(L32applicativeEval(vExp, env), v => makeOk<[string, Value]>([k, v])),
        exp.pairs
    );
    return bind(evaluatedPairsResult, (pairs: [string, Value][]) =>
