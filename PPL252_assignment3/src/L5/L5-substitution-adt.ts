...
import { isPairTExp, makePairTExp } from "./TExp";
...
export const checkNoOccurrence = (tvar: TVar, te: TExp): Result<true> => {
    const check = (e: TExp): Result<true> =>
        isTVar(e) ? ((e.var === tvar.var)
            ? bind(unparseTExp(te), up =>
                makeFailure(`Occur check error - circular sub ${tvar.var} in ${format(up)}`))
            : makeOk(true)) :
        isAtomicTExp(e) ? makeOk(true) :
        isProcTExp(e) ? bind(mapResult(check, e.paramTEs), _ => check(e.returnTE)) :
        isPairTExp(e) ? bind(check(e.t1), _ => check(e.t2)) :
...
export const applySub = (sub: Sub, te: TExp): TExp =>
    ...
    isProcTExp(te) ? makeProcTExp(
        map((t) => applySub(sub, t), te.paramTEs),
        applySub(sub, te.returnTE)) :
    isPairTExp(te) ? makePairTExp(
        applySub(sub, te.t1),
        applySub(sub, te.t2)) :
    te;
